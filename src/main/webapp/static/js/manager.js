...
app.directive("managerPanel", ['$http', function ($http) {
    var obj = {
        ...
        +'<div class="collapsible-body"><p>客栈名: {{checkout.hostelName}}</p><p>会员名: {{checkout.userName}}</p><p>开始时间: {{checkout.startData}}</p><p>结束时间 {{checkout.overData}}</p><p>每天价格: ¥{{bus.price}}</p><p>总计房费: ¥{{bus.cost}}</p><button class="waves-effect waves-light btn" style="margin: 15px; margin-left: 42%">结算</button>'
        +'</li>'
        +'</ul>'
        +'</div>'
        +'</div>'
        ...
        +'<div class="collapsible-body"><p>预定单号: {{bus.id}}</p><p>会员账号: {{bus.userName}}</p><p>会员卡号:{{bus.userId}}</p><p>开始时间: {{bus.startData}}</p><p>结束时间: {{bus.overData}}</p><p>每天价格: ¥{{checkout.price}}</p><p>总计房费: ¥{{checkout.cost}}</p></div>'
        +'</li>'
        +'</ul></div>'
        +'<div class="caption-container-main">'
